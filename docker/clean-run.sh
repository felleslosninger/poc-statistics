...
export VOLUME_EXISTS=$(docker volume ls | grep statistics-local-data | wc -l)

if [ $VOLUME_EXISTS -eq 1 ]
then
echo "Volume statistics-local-data exists, wait for dependent services/containers to be deleted before delete of volume succeeds..."
until docker volume rm statistics-local-data &> /dev/null; do sleep 1 ; done # Tries once a second to delete
fi
